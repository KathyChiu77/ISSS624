...
```{r}
pacman::p_load(sf, dplyr, sfdep, spdep, mapview, tmap, plotly, tidyverse, knitr, ggplot2)
```
...
hex_grid_sf <- hex_grid_sf %>%
  left_join(hex_weekday_morning_df, by = "hex_id")
```
...
# Perform the join using dplyr's left_join
hex_grid_sf <- hex_grid_sf %>%
  left_join(hex_weekday_afternoon_df, by = "hex_id")
```
...
# Perform the join using dplyr's left_join
hex_grid_sf <- hex_grid_sf %>%
  left_join(hex_weekend_morning_df, by = "hex_id")
```
...
# Perform the join using dplyr's left_join
hex_grid_sf <- hex_grid_sf %>%
  left_join(hex_weekend_evening_df, by = "hex_id")
```
...
hex_grid_sf <- hex_grid_sf %>%
  mutate(
    weekday_morning_trips = replace_na(weekday_morning_trips, 0),
    weekday_afternoon_trips = replace_na(weekday_afternoon_trips, 0),
    weekend_morning_trips = replace_na(weekend_morning_trips, 0),
    weekend_evening_trips = replace_na(weekend_evening_trips, 0)
  )
```
