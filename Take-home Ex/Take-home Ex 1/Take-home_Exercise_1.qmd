...
-   *Hexagon Layer*, a hexagon grid layer with a cell diameter of 250m, meant to replace the Master Plan 2019 Planning Sub-zone GIS data set of URA.

    Use the **`st_read`** function from the **`sf`** package to read in the existing geospatial layer, and then transform it to the correct coordinate reference system (CRS).
...
```{r}
hex_size <- 250 / sqrt(3) * 2  # Calculate the hexagon diameter
hex_grid <- st_make_grid(mpsz, cellsize = hex_size, square = FALSE)
```

Convert the grid into a simple feature collection with hexagon geometries.

```{r}
hex_sf <- st_sf(geometry = st_sfc(hex_grid, crs = st_crs(mpsz)))
```
